...
#--------------------------------------abs(tinhouse-tinhouse_demand) > 1 and tinhouse * t_rettown > 0 and
while  i<200:
    ...
    while j<20 and d_teta>0.05 : # приращение Т обратки в город меньше 0,5
        ...
        j +=1
        print(j, teta, d_teta) #-------------------------------------------
        #print('d_power %.2f.'% d_power, 'd_trettown %.3f.'% d_trettown, 'teta %.3f.' %teta)
    ...
    if i<100: #меняем входящую из города
        tintown=tintown + 0.05
